...
- `just build` — Debug build of `Cosmic Hammer.app` into `build/`. `just build Release` for Release. Orchestrates version numbering (from git tags), docs.json compilation, hs CLI build, xcodebuild, and post-build Lua/hs-CLI copying — no Xcode Run Script phases.
...
- `just test` — Runs the SPM test suite via `swift test` (requires a prior `just build` for Lua resources). Tests live in `Packages/CosmicHammerTests/` as a Swift Testing `.testTarget`.
...
Builds **must** go through the workspace, not the bare project. `xcodebuild -workspace CosmicHammer.xcworkspace -scheme CosmicHammer ...` (this is what the `justfile` does). Opening `CosmicHammer.xcodeproj` directly will fail because SPM resolution happens at the workspace level.
...
2. **Core app** (`CosmicHammer/`) — The `Cosmic Hammer.app` AppKit shell. `MJAppDelegate.m` boots the runtime; `MJLua.m` owns the `lua_State`, sets up `package.path`/`package.cpath`, and bootstraps `setup.lua` → `extensions/_coresetup/_coresetup.lua` → user `init.lua`. All `.m` files in this directory are compiled by SPM as part of the HSExtensions target (see below), not by the Xcode app target. The Xcode target's Sources build phase is empty — it only handles resources (XIBs, assets, plists).
...
All SPM code lives in a single unified package: **`Packages/Package.swift`**. It declares three internal targets (LuaSkin, CocoaHTTPServer, HSExtensions) and one product (`CosmicHammerLibs`). The `hs` CLI (`Packages/hs/`) is a separate, independent package built outside Xcode by the justfile.

Key pieces of this model — preserve them when adding extensions:

- `Packages/Package.swift` — Unified package. The `HSExtensions` target auto-discovers sources by following symlinks from `HSExtensions/Sources/HSExtensions/<name>/` into `extensions/<name>/` and from `HSExtensions/Sources/HSExtensions/CosmicHammer/` into the core app source tree. Non-source files inside `CosmicHammer/` (XIBs, plists, xcassets, entitlements, etc.) are individually excluded. `ipc/cli` (the standalone `hs` CLI) and `sqlite3/lsqlite3.c` (compiled indirectly via `lsqlite3_wrapper.m`) are also excluded. No need to edit Package.swift when adding extensions.
...
- `scripts/generate-hsextensions.sh` reads entry-point symbols from `extensions.manifest` and emits:
  ...
  - `CosmicHammer/HSExtensionsRegistry.m` (a `__attribute__((used))` static const function-pointer array — compiled as part of HSExtensions, in the same linkage unit as the `luaopen_*` symbols, preventing dead-stripping).
...
- `CosmicHammer/HSExecuteLuaIntent.h` and `HSExecuteLuaIntent.m` are pre-generated from `Intents.intentdefinition` (originally Xcode generated these at build time, but SPM cannot drive intent code generation). If the intent definition changes, regenerate them: open the workspace in Xcode, build once with the `.intentdefinition` in the Sources phase, then copy the generated files from DerivedData back into `CosmicHammer/`.
...
The Xcode project no longer needs per-extension targets — there are 2 Xcode targets (`CosmicHammer`, `CosmicHammerUITests`). Unit tests live in the SPM package as `CosmicHammerTests` (a `.testTarget` in `Packages/Package.swift`). The `hs` CLI is built by SPM (`Packages/hs/`) and copied into `Cosmic Hammer.app/Contents/Frameworks/hs/hs` by `just build` (post-build step).
