...
- **Granular Space and display lifecycle events** — Unlike Hammerspoon, `hs.spaces.watcher.newWithLifecycle()` reports Space creation/destruction with the ephemeral Space ID, and `hs.screen.watcher.newWithDisplayEvents()` reports display addition, removal, movement, and resizing with the display ID. Existing aggregate watcher APIs remain compatible; Space lifecycle delivery is experimental because it relies on private SkyLight notifications with a topology-diff fallback.
