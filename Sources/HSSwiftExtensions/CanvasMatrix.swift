...
/// [ 0,  0,  1 ]
/// ~~~
private func matrix_identity(_ L: LuaState) throws -> CInt {
...
private func matrix_invert(_ L: LuaState) throws -> CInt {
...
private func matrix_append(_ L: LuaState) throws -> CInt {
...
private func matrix_prepend(_ L: LuaState) throws -> CInt {
    let transform1 = toNSAffineTransformFromLua(L, idx: 1)
    let transform2 = toNSAffineTransformFromLua(L, idx: 2)
...
private func matrix_rotate(_ L: LuaState) throws -> CInt {
...
/// Returns:
///  * the new matrix
private func matrix_scale(_ L: LuaState) throws -> CInt {
    var transform = NSAffineTransform()
    var argsAt: Int32 = 2
    if lua_type(L, 1) == LUA_TNUMBER {
        argsAt = 1
...
/// Returns:
///  * the new matrix
private func matrix_shear(_ L: LuaState) throws -> CInt {
    var transform = NSAffineTransform()
    var argsAt: Int32 = 2
    if lua_type(L, 1) == LUA_TNUMBER {
        argsAt = 1
...
/// Returns:
///  * the new matrix
private func matrix_translate(_ L: LuaState) throws -> CInt {
    var transform = NSAffineTransform()
    var argsAt: Int32 = 2
    if lua_type(L, 1) == LUA_TNUMBER {
        argsAt = 1
...
public func luaopen_hs_libcanvasmatrix(_ L: UnsafeMutablePointer<lua_State>!) -> Int32 {
    runEntryPoint(L) { L in
        // Table-based metatable (matrix values are Lua tables, not userdata)
        luaL_newmetatable(L, USERDATA_TAG)
        lua_pushvalue(L, -1)
        lua_setfield(L, -2, "__index")
        L.push(USERDATA_TAG)
        lua_setfield(L, -2, "__type")
        L.push(matrix_rotate);    lua_setfield(L, -2, "rotate")
        L.push(matrix_translate); lua_setfield(L, -2, "translate")
        L.push(matrix_scale);     lua_setfield(L, -2, "scale")
        L.push(matrix_shear);     lua_setfield(L, -2, "shear")
        L.push(matrix_append);    lua_setfield(L, -2, "append")
        L.push(matrix_prepend);   lua_setfield(L, -2, "prepend")
        L.push(matrix_invert);    lua_setfield(L, -2, "invert")

        // Alias the metatable under the tag in the registry
        lua_setfield(L, LUA_REGISTRYINDEX_VALUE, USERDATA_TAG)

        // Create module table
        lua_createtable(L, 0, 8)
        L.push(matrix_identity);  lua_setfield(L, -2, "identity")
        L.push(matrix_rotate);    lua_setfield(L, -2, "rotate")
        L.push(matrix_translate); lua_setfield(L, -2, "translate")
        L.push(matrix_scale);     lua_setfield(L, -2, "scale")
        L.push(matrix_shear);     lua_setfield(L, -2, "shear")
