...
func MJLuaInit() {
    ...
    if lua_pcall(L, 7, 2, 0) != LUA_OK {
        ...
        if lua_gettop(L) != 2 || lua_type(L, -1) != LUA_TFUNCTION || lua_type(L, -2) != LUA_TFUNCTION {
            ...
            let errorMessage = "setup.lua failed to return the two items it is supposed to.\nThis is a severe bug. We would really appreciate your help in getting this fixed - please relaunch Cosmic Hammer so a crash report can be uploaded, then contact the developers via GitHub."
            let alert = NSAlert()
            alert.addButton(withTitle: "OK")
