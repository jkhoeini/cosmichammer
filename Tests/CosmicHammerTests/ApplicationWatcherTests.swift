...
extension CosmicHammerTests {
    @Suite(.serialized) @MainActor final class ApplicationWatcher {
        @Test func testApplicationWatcherActiveGauge() {
            withModuleLoaded(luaopen_hs_libapplicationwatcher) { L in
                ...
                let gaugeValues = sim.metrics
                    .filter { $0.name == "cosmichammer.application.watcher.active" }
                    .map(\.value)
