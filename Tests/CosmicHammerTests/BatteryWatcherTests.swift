...
extension CosmicHammerTests {
    @Suite(.serialized) @MainActor final class BatteryWatcher {
        @Test func testBatteryWatcherActiveGauge() {
            withModuleLoaded(luaopen_hs_libbatterywatcher) { L in
                ...
                let gaugeValues = sim.metrics
                    .filter { $0.name == "cosmichammer.battery.watcher.active" }
                    .map(\.value)
