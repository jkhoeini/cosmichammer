...
extension CosmicHammerTests {
    @Suite(.serialized) @MainActor final class FileSystemVolume {
        @Test func testFileSystemVolumeWatcherActiveGauge() {
            withModuleLoaded(luaopen_hs_libfsvolume) { L in
                ...
                let gaugeValues = sim.metrics
                    .filter { $0.name == "cosmichammer.fs.volume.watcher.active" }
                    .map(\.value)
