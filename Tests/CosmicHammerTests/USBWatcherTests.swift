...
extension CosmicHammerTests {
    @Suite(.serialized) @MainActor final class USBWatcher {
        @Test func testUSBWatcherActiveGauge() {
            withModuleLoaded(luaopen_hs_libusbwatcher) { L in
                ...
                let gaugeValues = sim.metrics
                    .filter { $0.name == "cosmichammer.usb.watcher.active" }
                    .map(\.value)
