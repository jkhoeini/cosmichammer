...
extension CosmicHammerTests {
    @Suite(.serialized) @MainActor final class WebviewNavigationCallback {
        @Test(.skipInHeadless) func testWebviewNavigationCallbackActiveGauge() {
            ...
            let result = runLua("""
                ...
                view:navigationCallback(function() end)
                view:navigationCallback(function() end)
                view:navigationCallback(nil)
                view:navigationCallback(nil)
                view:delete()
                return "ok"
            """)
            ...
            let gaugeValues = sim.metrics
                .filter { $0.name == "cosmichammer.webview.navigation.callback.active" }
                .map(\.value)
