...
extension CosmicHammerTests {
    @Suite(.serialized) @MainActor final class WifiWatcher {
        @Test func testWifiWatcherActiveGauge() {
            withModuleLoaded(luaopen_hs_libwifiwatcher) { L in
                ...
                let gaugeValues = sim.metrics
                    .filter { $0.name == "cosmichammer.wifi.watcher.active" }
                    .map(\.value)
