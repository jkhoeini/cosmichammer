...
function testDataSourceTypeMetadata()
  for _, device in ipairs(hs.audiodevice.allDevices()) do
    local sources = {}
    if device:supportsOutputDataSources() then
      sources = device:allOutputDataSources() or {}
    end
    if #sources == 0 and device:supportsInputDataSources() then
      sources = device:allInputDataSources() or {}
    end
    if #sources > 0 then
      assertIsUserdataOfType("hs.audiodevice.datasource", sources[1])
      return success()
    end
  end
