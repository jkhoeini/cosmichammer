...
-- cosmic hammer is single threaded, thus this does not need a semaphore
...
---    * initial - this color is used for the initial tagline when starting the command line tool and for output to the Cosmic Hammer console that is redirected to the instance.  Defaults to "\27[35m" (foreground magenta).
...
---
---  * Changes made with this function are saved with `hs.settings` with the following labels and will persist through a reload or restart of Cosmic Hammer: "ipc.cli.color_initial", "ipc.cli.color_input", "ipc.cli.color_output", and "ipc.cli.color_error"
...
---  * silent - An optional boolean indicating whether or not to print errors to the Cosmic Hammer Console
---
--- Returns:
...
module.cliStatus = function(p, s)
    ...
    if bin_file and not bin_link then
        if not silent then
            print([[cli installation problem: 'hs' is an independent file won't be updated when Cosmic Hammer is.]])
        end
        broken = true
    end
    ...
    if man_file and not man_link then
        if not silent then
            print([[cli installation problem: man page for 'hs.1' is an independent file and won't be updated when Cosmic Hammer is.]])
        end
        broken = true
    end
...
module.__default = module.localPort("Cosmic Hammer", module.__defaultHandler)
